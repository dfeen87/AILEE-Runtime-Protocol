...
ZKProofArtifact Halo2Backend::generate_proof(
...
bool Halo2Backend::verify_proof(
    const ZKBackendConfig& config,
...
ZKProofArtifact Halo2Backend::generate_recursive_proof(
    const ZKBackendConfig& config,
    const ZKConstraintSet& constraints,
    const ZKTranscript& transcript,
    ...
    throw std::runtime_error("Halo2Backend: Native stub recursive proof generation not yet implemented.");
}

bool Halo2Backend::verify_recursive_proof(
    const ZKBackendConfig& config,
    const ZKProofArtifact& artifact,
    const ZKConstraintSet& constraints,
    const ZKTranscript& transcript,
    ...
    (void)transcript;
    (void)recursion_bundle;
    throw std::runtime_error("Halo2Backend: Native stub recursive proof validation not yet implemented.");
}
