...
IslaRuntimeOrchestrator::IslaRuntimeOrchestrator(const RuntimeEnvironment& env)
    ...
    energy_coordinator_ = std::make_unique<erl::TemporalEnergyCoordinator>(sem_config);
    iao_ = std::make_unique<iao::IntelligenceAssistedOrchestrator>();
}
...
IslaEpochResult IslaRuntimeOrchestrator::run_epoch(const EpochIntegrationBundle& bundle) {
    ...
    if (temporal_auditor_ && temporal_buffer_) {
        ...
        erl::EnergyAdaptationDecision energy_decision;
        if (energy_coordinator_) {
            energy_decision = energy_coordinator_->evaluate(bundle.hice_metrics, validity_surface);
        }
        if (iao_) {
            final_result.guidance_report = iao_->evaluate(
                *temporal_buffer_,
                validity_surface,
                energy_decision
            );
